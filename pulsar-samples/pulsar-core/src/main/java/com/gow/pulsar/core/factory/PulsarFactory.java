package com.gow.pulsar.core.factory;
...
public class PulsarFactory {
    ...
    private void configClient(ClientBuilder builder,
        ...
        Optional.ofNullable(clientProperties).ifPresent(properties -> {
            ...
            builder.statsInterval(properties.getStatsIntervalSeconds(), TimeUnit.SECONDS);
        });
    }
    ...
    private <T> ConsumerBuilder<T> configConsumer(ConsumerBuilder<T> consumerBuilder,
        ...
        return Optional.ofNullable(consumer).map(consumerProperties -> {
            ...
            Optional.ofNullable(consumer.getMaxRedeliverCount()).ifPresent(count -> {
                consumerBuilder
                        .deadLetterPolicy(DeadLetterPolicy.builder().deadLetterTopic(consumer.getDeadLetterTopic())
                                .retryLetterTopic(consumer.getRetryLetterTopic())
                                .maxRedeliverCount(consumer.getMaxRedeliverCount()).build());
            });
            ...
            return consumerBuilder.receiverQueueSize(consumer.getReceiverQueueSize())
                    .batchReceivePolicy(consumer.getBatchReceivePolicy())
