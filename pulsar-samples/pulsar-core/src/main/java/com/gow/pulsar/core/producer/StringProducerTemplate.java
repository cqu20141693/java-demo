...
public class StringProducerTemplate implements DisposableBean {
    ...
    private boolean check(ProducerRecord<String> record, Producer<String> producer) {
        if (producer == null) {
            PulsarLog.log.warn("producer not exist for topic={}", record.topic());
            return true;
        }
        if (!producer.isConnected()) {
            PulsarLog.log.warn("producer is disconnected topic={}", record.topic());
            return true;
        }
        return false;
    }
