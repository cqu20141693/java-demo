...
public final class Topic<T> {
    ...
    public List<Topic<T>> findTopic(String topic) {
        ArrayList<Topic<T>> list = new ArrayList<>();
        findTopic(topic, list::add, () -> {
        });
        return list;
    }
    ...
    public void findTopic(String topic,
        ...
        find(TopicUtils.split(topic), this, sink, () -> {
        });
    }

    public void findTopic(String topic,
                          Consumer<Topic<T>> sink, Runnable end) {
        if (!topic.startsWith("/")) {
            topic = "/" + topic;
        }
...
//    }
    public List<Topic<T>> getAllSubscriber() {
        ...
        List<Topic<T>> all = new ArrayList<>(this.getChildren());

        for (Topic<T> tTopic : getChildren()) {
            all.addAll(tTopic.getAllSubscriber());
        }
        return all;
    }
