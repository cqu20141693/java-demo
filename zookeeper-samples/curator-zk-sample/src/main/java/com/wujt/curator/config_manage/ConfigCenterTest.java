...
public class ConfigCenterTest {
            ...
            new Thread(new Runnable() {
                ...
                public void run() {
                    ...
                    try {
                        ...
                        // 设置监听，修改后理解重新执行查询操作
                        cacheNode.getListenable().addListener(() -> {
                            byte[] newData = cacheNode.getCurrentData().getData();
                            Stat stat1 = cacheNode.getCurrentData().getStat();
                            if (stat1.getMzxid() == stat[0].getMzxid()) {
                                return;
                            }
                            stat[0] = stat1;
    ...
    private static void select(MysqlConfig mysqlConfig) throws ClassNotFoundException, SQLException {
