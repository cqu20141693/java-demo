...
public class DefaultCertificate implements Certificate {
    ...
    public X509Certificate[] getCertificateChain(String serverName) {
        return Optional
                .ofNullable(getX509KeyManager(serverName))
                .map(manager -> manager.getCertificateChain(null))
                .orElse(null);
    }
    ...
    public String toString() {
        return Stream.of(getX509KeyManagers())
                .flatMap(x509KeyManager -> Stream.of(x509KeyManager.getCertificateChain(null)))
    ...
    public <T> T doWithKeyStore(BiFunction<KeyStore, String, T> function) {
        return keyHelper.doWithKeyStore(function);
    }
}
