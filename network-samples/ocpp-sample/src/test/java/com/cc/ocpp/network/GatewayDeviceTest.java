...
import com.cc.ocpp.network.cp.domian.NetworkPropertiesEntity;
import com.cc.ocpp.network.cp.server.CPNetworkProvider;
...
class GatewayDeviceTest {
    ...
    public void testCreateGateWayDevice() {
        ...
        Optional.of(createEntity(type, true, new HashMap<>()))
                .map(this::toProperties)
                .filter(NetworkProperties::isEnabled)
                .map(provider::createConfig)
