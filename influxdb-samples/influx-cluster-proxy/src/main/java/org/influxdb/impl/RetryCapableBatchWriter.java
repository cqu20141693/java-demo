...
class RetryCapableBatchWriter implements BatchWriter {
  ...
  private BiConsumer<BatchPoints, Throwable> exceptionHandler;
  ...
  RetryCapableBatchWriter(final InfluxDB influxDB, final BiConsumer<BatchPoints, Throwable> exceptionHandler,
    ...
    while (batchQueueIterator.hasNext()) {
      ...
      if (result.outcome == WriteResultOutcome.WRITTEN
        ...
        if (result.outcome == WriteResultOutcome.FAILED_RETRY_IMPOSSIBLE) {
          exceptionHandler.accept(entry, result.throwable);
        }
      } else {
    ...
    while (collectionIterator.hasNext()) {
      ...
      switch (result.outcome) {
        ...
        case FAILED_RETRY_IMPOSSIBLE:
          exceptionHandler.accept(batchPoints, result.throwable);
          break;
        default:
  ...
  public synchronized void close() {
    ...
    for (BatchPoints points : batchQueue) {
      ...
      if (result.outcome != WriteResultOutcome.WRITTEN) {
        exceptionHandler.accept(points, result.throwable);
      }
    }
  }
  ...
  private void evictTooOldFailedWrites() {
    while (usedRetryBufferCapacity > retryBufferCapacity && batchQueue.size() > 0) {
      BatchPoints batchPoints = batchQueue.removeFirst();
      List<Point> points = batchPoints.getPoints();
      usedRetryBufferCapacity -= points.size();
      exceptionHandler.accept(batchPoints,
